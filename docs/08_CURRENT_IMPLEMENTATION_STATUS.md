...
> **Last updated:** September 14, 2026 v12 · Branch `main` · **Immediate Alert Voice Silencing & Acknowledge Retention, SafeZone HexaCore Favicon, Netlify Deployment Readiness & Full Containerized Docker Stack Complete**
>
...
| "Mitra" Crisis Companion | Neural Voice + GSAP + AI | Contextual coaching in sim, Gemini 1.5 Flash API with offline rule-based fallback. Dual-tier voice engine: zero-latency Web Speech API with tuned natural pitch (1.0) and rate (0.96), with auto-fallback to Microsoft Neural Studio TTS (`edge-tts` `en-IN-NeerjaNeural` / `en-US-JennyNeural`) and token-guarded instantaneous silence on acknowledgment. (Issue 4.3, v11, v12) |
| Backend & Cloud Infrastructure | Built | DynamicPathfinder (<15ms A*), FastAPI WebSocket hub, PostgreSQL telemetry persistence (`POST /runs`, `GET /analytics`), UserProfile & NDMAReport models, live disaster alerts feed (`GET /alerts/live`), incident injection webhooks, background SACHET CAP poller, Neural TTS streaming endpoint (`GET/POST /api/v1/mitra/tts`), 4-tier Docker Compose orchestration (PostGIS, Redis, Backend, Frontend), and Netlify production deployment readiness (`netlify.toml`). (Tasks 3.1, 3.3, 3.4, PR #28, v11, v12) |
